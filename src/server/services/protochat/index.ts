 ...
 * ProtoChat Service
 *
 * Provides functionality for model mapping, pricing queries, usage logging, etc.
 */
export class ProtoChatService {
  ...
  async getModelMapping(modelId: string): Promise<ProtoChatModelMapping> {
    ...
    // If exact match fails, try fuzzy matching (supports legacy model ID formats)
    if (!model.length && !modelId.startsWith('protochat::')) {
      // Try to match full ID ending with the given model ID (e.g. 'protochat::a1::gemini-2.5-flash' matches 'gemini-2.5-flash')
      model = await this.db
        .select()
        .from(protochatModels)
        .where(
  ...
  /**
   * Get list of all enabled ProtoChat models
   * Used by the main project to retrieve model list
   */
