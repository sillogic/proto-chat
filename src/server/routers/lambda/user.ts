...
export const userRouter = router({
  ...
  getUserState: userProcedure.query(async ({ ctx }): Promise<UserInitializationState> => {
    ...
    return {
      ...
      // If there are messages or an assistant has been created, consider the conversation as existing
  ...
  updateAvatar: userProcedure.input(z.string()).mutation(async ({ ctx, input }) => {
    // If the data is Base64-encoded, upload it to S3
    if (input.startsWith('data:image')) {
      try {
        ...
        // Generate a unique filename using UUID to prevent cache issues
