...
export const imageRouter = router({
  createImage: imageProcedure.input(createImageInputSchema).mutation(async ({ input, ctx }) => {
    ...
    // 1) Handle multiple image URLs (imageUrls)
    ...
    // 2) Handle single image URL (imageUrl)
    ...
    const { batch: createdBatch, generationsWithTasks } = await serverDB.transaction(async (tx) => {
      ...
      // 2. Create 4 generations (phase 1 fixed: generate 4 images)
      const seeds =
        'seed' in params
      ...
      const generationsWithTasks = await Promise.all(
        createdGenerations.map(async (generation) => {
          ...
          // Update the generation's asyncTaskId
          await tx
            .update(generations)
            .set({ asyncTaskId })
    ...
    try {
      ...
      // Create caller using the unified caller factory
