 ...
 * to deliver a unified AI service with credit-based billing.
 *
 * Characteristics:
 * - Model list is dynamically fetched from the backend database
 * - Does not support user-defined API Keys or Base URLs
 * - Only backend administrators can configure the underlying providers
 */
const ProtoChat: ModelProviderCard = {
  // Model list is fetched dynamically from the database; empty here
  chatModels: [],
  // No model check needed since this is a managed service
  checkModel: undefined,
  description:
    'ProtoChat AI 服务是由系统管理员统一配置和管理的AI服务。提供多种主流AI模型，支持对话、图像生成等功能。使用此服务将消耗积分。',
  // Enabled by default so new users can use it immediately
  enabled: true,
  // Provider ID
  id: 'protochat',
  // Model list configuration
  modelList: {
    // Do not show the model fetcher button since the model list is managed by the backend
    showModelFetcher: false,
  },
  // Provider name
  name: 'ProtoChat AI',
  settings: {
    // Disable browser-side requests (proxied through the server)
