 ...
 * @returns The parsed feature flag object.
 */
export function parseFeatureFlag(flagString?: string): Partial<IFeatureFlags> {
  ...
  // Replace Chinese commas with English commas, then split the string by comma
  ...
  for (let flag of flagArray) {
    ...
    if (flag.startsWith('+') || flag.startsWith('-')) {
      ...
      // Check if key exists in FeatureFlagsSchema
