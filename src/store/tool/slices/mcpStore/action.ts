...
export interface PluginMCPStoreAction {
  ...
  // Methods related to connection testing
...
> = (set, get) => ({
  ...
  installMCPPlugin: async (identifier, options = {}) => {
    ...
    try {
      ...
      if (resume) {
        ...
        if (hasCloudEndpoint) {
          ...
          if (result.needsConfig) {
            ...
            // Pause installation, wait for user to configure
            updateInstallLoadingState(identifier, undefined);
      ...
      // set version
      if (manifest) {
        // set Version - compare versions using semver and take the larger value
      ...
      // If the error is due to cancellation, handle silently
      if (abortController.signal.aborted) {
      ...
      if (!!error.data && 'errorData' in error.data) {
        ...
        // Fallback for handling generic errors
