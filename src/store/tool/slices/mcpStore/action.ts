...
const buildCloudMcpManifest = (params: {
  ...
  // Get tools from data (MCP format) or api (LobeChat format)
...
> = (set, get) => ({
  ...
  installMCPPlugin: async (identifier, options = {}) => {
    ...
    try {
      ...
      if (resume) {
        ...
        if (hasCloudEndpoint) {
          ...
          if (!skipDepsCheck && !result.allDependenciesMet) {
            ...
            updateInstallLoadingState(identifier, undefined);
            return false; // Return false to indicate dependencies are needed
          }
      ...
      // Show completion status briefly before clearing progress
      await sleep(1000);
      ...
      // Handle silently if error is due to cancellation
      if (abortController.signal.aborted) {
