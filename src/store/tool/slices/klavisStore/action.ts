...
> = (set, get) => ({
  ...
  createKlavisServer: async (params) => {
    ...
    try {
      ...
      set(
        produce((draft: KlavisStoreState) => {
          // Check if it already exists (using identifier), update if it does
    ...
    try {
      ...
      if (!instanceStatus.isAuthenticated) {
        if (!instanceStatus.authNeeded) {
          // If authentication is not needed, then it's fine
          return;
        }
      ...
      // Authentication succeeded, get tool list (use toolsClient for longer timeout)
