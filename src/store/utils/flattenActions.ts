/**
 * Flattens multiple action objects (typically class instances) into a single plain object
 *
 * Solves the issue where class instances cannot correctly copy prototype methods via the spread operator
  ...
  for (const action of actions) {
    ...
    while (current && current !== Object.prototype) {
      ...
      for (const key of keys) {
        ...
        if (key in result) continue; // Skip already-existing properties (prefer the first action's method)
