...
const KlavisServerItem = memo<KlavisServerItemProps>(
  ({ identifier, label, server, serverName, agentId }) => {
    ...
    const startWindowMonitor = useCallback(
      (oauthWindow: Window, serverName: string) => {
        ...
        windowCheckIntervalRef.current = setInterval(() => {
          try {
            // Attempt to access window.closed (may be blocked by COOP)
            if (oauthWindow.closed) {
              // Window is closed, clean up listeners and check auth status
              if (windowCheckIntervalRef.current) {
                clearInterval(windowCheckIntervalRef.current);
                windowCheckIntervalRef.current = null;
              }
    ...
    const openOAuthWindow = useCallback(
      (oauthUrl: string, serverName: string) => {
        ...
        if (oauthWindow) {
          ...
          // Window was blocked, fall back to polling directly
          startFallbackPolling(serverName);
        }
      },
    ...
    // Get plugin ID for this server (using identifier as pluginId)
    ...
    const handleConnect = async () => {
      ...
      try {
        ...
        if (newServer) {
          ...
          // If already authenticated, refresh tools directly, skip OAuth
          if (newServer.isAuthenticated) {
            await refreshKlavisServerTools(newServer.identifier);
          } else if (newServer.oauthUrl) {
            // OAuth required, open OAuth window and monitor for close
