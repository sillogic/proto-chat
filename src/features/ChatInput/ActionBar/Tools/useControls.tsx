...
export const useControls = ({ setUpdating }: { setUpdating: (updating: boolean) => void }) => {
  ...
  // Using KLAVIS_SERVER_TYPES instead of connected servers since we need to filter out all possible Klavis types
  ...
  // Function to map a plugin to a list item
  ...
  // Build children for the LobeHub group (includes builtin tools and LobeHub Skill/Klavis)
  ...
  const installedPluginItems: ItemType[] = useMemo(() => {
    ...
    // Merge enabled LobeHub Skill and Klavis items (as builtin skills)
    const enabledSkillItems = [...connectedLobehubSkillItems, ...connectedKlavisItems];

    // Build children for the builtin tools group (includes builtin tools and LobeHub Skill/Klavis)
    const allBuiltinItems: ItemType[] = [
      // 1. Builtin tools
      ...enabledBuiltinItems,
      // 2. Divider (if there are both builtin tools and skill items)
