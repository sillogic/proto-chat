...
exports.cleanComfyUIErrorMessage = cleanComfyUIErrorMessage;
exports.parseComfyUIErrorMessage = parseComfyUIErrorMessage;
 ...
 */
function cleanComfyUIErrorMessage(message) {
    return message
 ...
 */
function extractComfyUIErrorInfo(error) {
    ...
    if (error instanceof Error) {
        ...
        return {
            ...
            missingFileName: error.missingFileName,
            missingFileType: error.missingFileType,
    ...
    if (error && typeof error === 'object') {
        ...
        const missingFileName = error.missingFileName || error.body?.error?.missingFileName || error.error?.missingFileName;
        const missingFileType = error.missingFileType || error.body?.error?.missingFileType || error.error?.missingFileType;
        const userGuidance = error.userGuidance || error.body?.error?.userGuidance || error.error?.userGuidance;
        return {
            code,
            details,
            message: cleanedMessage,
 ...
 */
function parseComfyUIErrorMessage(error) {
    ...
    // Default error type
    let errorType = error_1.AgentRuntimeErrorType.ComfyUIBizError;
    ...
    switch (status) {
        ...
        case 403: {
            // Permission denied
            errorType = error_1.AgentRuntimeErrorType.PermissionDenied;
            break;
        }
        case 404: {
        ...
        default: {
            if (status && status >= 500) {
                // Server errors
                errorType = error_1.AgentRuntimeErrorType.ComfyUIServiceUnavailable;
            }
...
//# sourceMappingURL=comfyuiErrorParser.js.map