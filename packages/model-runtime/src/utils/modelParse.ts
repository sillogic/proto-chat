...
// Default keyword: any model ID containing -search is considered to support online search
...
/**
 * Check whether a keyword list matches a model ID (supports multiple matching modes)
 ...
 * @returns Whether there is a match (exclusion logic takes priority)
 */
const isKeywordListMatch = (modelId: string, keywords: readonly string[]): boolean => {
  // Check exclusion rules first (those starting with !)
...
/**
 * Find the local model configuration corresponding to the given provider type
...
const findKnownModelByProvider = async (
  ...
  try {
    ...
    // Skip if the provider configuration file does not exist
    ...
    // If import succeeds and data is available, perform lookup
    ...
    // Return null if import fails (file not found or other error)
    return null;
  }
};
 ...
 * @param modelId Model ID
 * @returns Detected provider configuration key, defaults to 'openai'
 */
...
const formatTimestampToDate = (timestamp: number): string | undefined => {
  ...
  // - If millisecond-level (>= 1e12), use as-is;
  // - Otherwise treat as seconds and multiply by 1000 to convert to milliseconds
  const msTimestamp = timestamp > 1e12 ? timestamp : timestamp * 1000;
  const date = new Date(msTimestamp);

  // Validate parsed result and year range (only accept 4-digit years, to stay within YYYY-MM-DD varchar(10))
...
const processReleasedAt = (model: any, knownModel?: any): string | undefined => {
  ...
  if (model.created !== undefined && model.created !== null) {
    // Check whether it is a timestamp format
    if (typeof model.created === 'number' && model.created > 1_630_000_000) {
      // AiHubMix erroneous timestamp is 1626777600
    ...
    if (typeof model.created === 'string') {
      // Anthropic: if the value is '2025-02-19T00:00:00Z', extract only the date part
...
const processDisplayName = (displayName: string): string => {
  // If the name contains "Gemini 2.5 Flash Image Preview", replace that part with "Nano Banana"
...
/**
 * Get the local configuration for a model provider
...
const getProviderLocalConfig = async (provider?: ModelProviderKey): Promise<any[] | null> => {
  ...
  if (provider) {
    try {
      ...
      // Keep as null if configuration file does not exist or import fails
...
/**
 * Get the local configuration for a model
...
const getModelLocalEnableConfig = (
  ...
  // If providerid is provided and local config exists, attempt to retrieve the model's enabled state from it
 ...
 * @param provider Provider type (optional; used to preferentially match the corresponding local config; only when provider is supplied will the enabled state be overridden from local config)
...
export const processModelList = async (
  ...
  // If provider is supplied, attempt to get the local configuration for that provider
  ...
  return Promise.all(
    modelList.map(async (model) => {
      ...
      // If provider is supplied, prefer provider-specific configuration
      if (provider) {
      ...
      // If provider is supplied and local config exists, attempt to retrieve the model's enabled state from it
...
/**
 * Process a model list containing models from multiple providers
 * @param modelList Model list
 * @param providerid Optional provider ID used to fetch the provider's local config file
 * @returns Processed list of model cards
 */
export const processMultiProviderModelList = async (
  ...
  // If providerid is supplied, attempt to get the local configuration for that provider
  ...
  return Promise.all(
    modelList.map(async (model) => {
      ...
      // Prefer provider-specific configuration
      ...
      // If providerid is supplied and local config exists, attempt to retrieve the model's enabled state from it
      const providerLocalModelConfig = getModelLocalEnableConfig(
        providerLocalConfig as any[],
        model,
      );
