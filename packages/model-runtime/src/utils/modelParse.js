...
exports.processMultiProviderModelList = exports.processModelList = exports.detectModelProvider = exports.EMBEDDING_MODEL_KEYWORDS = exports.IMAGE_MODEL_KEYWORDS = exports.MODEL_OWNER_DETECTION_CONFIG = exports.MODEL_LIST_CONFIGS = void 0;
// 默认关键字：任意包含 -search 的模型 ID 视为支持联网搜索
const DEFAULT_SEARCH_KEYWORDS = ['-search'];
// 模型能力标签关键词配置
exports.MODEL_LIST_CONFIGS = {
    anthropic: {
...
// 模型所有者 (提供商) 关键词配置
exports.MODEL_OWNER_DETECTION_CONFIG = {
    anthropic: ['claude'],
...
// 嵌入模型关键词配置
exports.EMBEDDING_MODEL_KEYWORDS = ['embedding', 'embed', 'bge', 'm3e'];
/**
 ...
 * @returns 是否匹配（排除逻辑优先）
 */
const isKeywordListMatch = (modelId, keywords) => {
    // 先检查排除规则（感叹号开头）
 ...
 * @returns 匹配的本地模型配置
 */
const findKnownModelByProvider = async (modelId, provider) => {
    const lowerModelId = modelId.toLowerCase();
    try {
        // 尝试动态导入对应的配置文件
        const modules = await Promise.resolve().then(() => __importStar(require('model-bank')));
        // 如果提供商配置文件不存在，跳过
 ...
 */
const detectModelProvider = (modelId) => {
    const lowerModelId = modelId.toLowerCase();
    for (const [provider, keywords] of Object.entries(exports.MODEL_OWNER_DETECTION_CONFIG)) {
        const hasKeyword = isKeywordListMatch(lowerModelId, keywords);
        if (hasKeyword && provider in exports.MODEL_LIST_CONFIGS) {
            return provider;
        }
    }
    return 'openai';
};
exports.detectModelProvider = detectModelProvider;
/**
 * 将时间戳转换为日期字符串
 ...
 */
const processReleasedAt = (model, knownModel) => {
    // 优先检查 model.created
    if (model.created !== undefined && model.created !== null) {
        // 检查是否为时间戳格式
        if (typeof model.created === 'number' && model.created > 1630000000) {
 ...
 * @returns 模型提供商的本地配置
 */
const getProviderLocalConfig = async (provider) => {
    let providerLocalConfig = null;
    if (provider) {
        try {
            const modules = await Promise.resolve().then(() => __importStar(require('model-bank')));
 ...
 * @returns 模型本地配置
 */
const getModelLocalEnableConfig = (providerLocalConfig, model) => {
 ...
 * 处理模型卡片的通用逻辑
 */
const processModelCard = (model, config, knownModel) => {
    const { functionCallKeywords = [], visionKeywords = [], reasoningKeywords = [], excludeKeywords = [], searchKeywords = DEFAULT_SEARCH_KEYWORDS, imageOutputKeywords = [], videoKeywords = [], } = config;
    const isExcludedModel = isKeywordListMatch(model.id.toLowerCase(), excludeKeywords);
    const modelType = model.type ||
        knownModel?.type ||
        (isKeywordListMatch(model.id.toLowerCase(), exports.IMAGE_MODEL_KEYWORDS.map((k) => k.toLowerCase()))
            ? 'image'
            : isKeywordListMatch(model.id.toLowerCase(), exports.EMBEDDING_MODEL_KEYWORDS.map((k) => k.toLowerCase()))
                ? 'embedding'
                : 'chat');
 ...
 * @returns 处理后的模型卡片列表
 */
const processModelList = async (modelList, config, provider) => {
    ...
    return Promise.all(modelList.map(async (model) => {
        ...
        // 如果未找到，回退到全局配置
        if (!knownModel) {
            knownModel = LOBE_DEFAULT_MODEL_LIST.find((m) => model.id.toLowerCase() === m.id.toLowerCase());
        }
        const processedModel = processModelCard(model, config, knownModel);
        // 如果提供了 provider 且有本地配置，尝试从中获取模型的 enabled 状态
        const providerLocalModelConfig = getModelLocalEnableConfig(providerLocalConfig, model);
 ...
 * @returns 处理后的模型卡片列表
 */
const processMultiProviderModelList = async (modelList, providerid) => {
    const { LOBE_DEFAULT_MODEL_LIST } = await Promise.resolve().then(() => __importStar(require('model-bank')));
    // 如果提供了 providerid，尝试获取该提供商的本地配置
    const providerLocalConfig = await getProviderLocalConfig(providerid);
    return Promise.all(modelList.map(async (model) => {
        const detectedProvider = (0, exports.detectModelProvider)(model.id);
        const config = exports.MODEL_LIST_CONFIGS[detectedProvider];
        // 优先使用提供商特定的配置
...
exports.processMultiProviderModelList = processMultiProviderModelList;
//# sourceMappingURL=modelParse.js.map