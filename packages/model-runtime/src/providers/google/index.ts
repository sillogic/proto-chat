...
export class LobeGoogleAI implements LobeRuntimeAI {
  ...
  async chat(rawPayload: ChatStreamPayload, options?: ChatMethodOptions) {
    try {
      ...
      // Remove previous silent handling and throw errors uniformly
      if (isAbortError(err)) {
        ...
        try {
          for await (const chunk of originalStream) {
            if (signal.aborted) {
              ...
              if (hasData) {
                ...
                // Explicitly inject a cancellation error to avoid falling back to SSE unexpected_end
                controller.enqueue({
                  [LOBE_ERROR_KEY]: {
                ...
                // If no data has been output yet, close the stream directly and let the downstream SSE emit an error event during flush
          ...
          if (isAbortError(err) || signal.aborted) {
            ...
            if (hasData) {
              ...
              // Inject an error marker with detailed error information for the downstream google-ai transformer to emit an error event
              controller.enqueue({
                [LOBE_ERROR_KEY]: {
                  body: {
                    message: err.message,
            ...
            // Attempt to parse Google error and extract code/message/status
  ...
  private buildGoogleToolsWithSearch(
    ...
    // Build and return search-related tools (search tools cannot be used together with FunctionCall)
    ...
    // Consider function declarations last
    return buildGoogleTools(tools);
  }
}
