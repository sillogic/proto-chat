...
exports.LobeReplicateAI = void 0;
...
class LobeReplicateAI {
    constructor({ apiKey, baseURL = DEFAULT_BASE_URL, id } = {}) {
        if (!apiKey) {
    ...
    async createImage(payload) {
        try {
            ...
            if (imageUrl) {
                ...
                if (isLocalUrl) {
                    this.debugLog('[Replicate createImage] Local URL detected, will fetch and upload as data');
                    try {
                        const { ssrfSafeFetch } = await Promise.resolve().then(() => __importStar(require('ssrf-safe-fetch')));
                        ...
                        this.debugLog('[Replicate createImage] Fetched image, size:', imageBuffer.length, 'bytes');
            ...
            this.debugLog('[Replicate createImage] Final input object:', JSON.stringify(inputForLogging, null, 2));
            ...
            this.debugLog('[Replicate createImage] Output type:', typeof output, 'Is array:', Array.isArray(output));
    ...
    async models() {
        try {
            ...
            return (0, modelParse_1.processModelList)(modelList, modelParse_1.MODEL_LIST_CONFIGS.replicate, 'replicate');
        }
        catch (error) {
            throw this.handleError(error);
        }
     ...
     */
    debugLog(...args) {
        const isReplicateDebug = process.env.DEBUG_REPLICATE === '1' ||
...
exports.LobeReplicateAI = LobeReplicateAI;
exports.default = LobeReplicateAI;
