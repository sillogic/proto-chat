...
exports.createQwenImage = createQwenImage;
const debug_1 = __importDefault(require("debug"));
const asyncifyPolling_1 = require("../../utils/asyncifyPolling");
const createError_1 = require("../../utils/createError");
const log = (0, debug_1.default)('lobe-image:qwen');
/**
...
async function createImageTask(payload, apiKey) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to create image task (${response.status}): ${errorData?.message || response.statusText}`);
    }
 ...
 */
async function createImageEdit(payload, apiKey) {
    const { model, params } = payload;
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to create image edit (${response.status}): ${errorData?.message || response.statusText}`);
    }
    const data = await response.json();
 ...
 */
async function queryTaskStatus(taskId, apiKey) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to query task status (${response.status}): ${errorData?.message || response.statusText}`);
    }
 ...
 */
async function createQwenImage(payload, options) {
    ...
    try {
        ...
        const result = await (0, asyncifyPolling_1.asyncifyPolling)({
