  ...
  models: async () => {
    ...
    const formattedModels = modelList
      .map((model) => {
        ...
        // Priority for selecting provider info: is_model_author > info completeness > default first
        const mainProvider =
        ...
        // Multi-provider fallback strategy: get from main provider first, fall back to other providers if missing
        ...
        // displayName uses id with content left of slash removed (e.g. 'zai-org/GLM-4.6' -> 'GLM-4.6')
        const displayName =
