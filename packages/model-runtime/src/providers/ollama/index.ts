...
export class LobeOllamaAI implements LobeRuntimeAI {
  ...
  async pullModel(params: PullModelParams, options?: ModelRequestOptions): Promise<Response> {
    ...
    const signal = options?.signal; // Get the provided AbortSignal

    const abortOllama = () => {
      // Assumes this.client.abort() is idempotent or can be safely called multiple times
      this.client.abort();
    };

    // If AbortSignal is provided, listen for the abort event
    // Use { once: true } to ensure the listener fires only once
    signal?.addEventListener('abort', abortOllama, { once: true });

    try {
      // Get the iterator for the Ollama pull
      ...
      const progressStream = createModelPullStream(iterable, model, {
        onCancel: () => {
          ...
          // Remove the signal listener to avoid duplicate calls (in case abortOllama is not idempotent)
      ...
      // If an error occurs when calling client.pull or creating the stream, remove the listener
      signal?.removeEventListener('abort', abortOllama);
