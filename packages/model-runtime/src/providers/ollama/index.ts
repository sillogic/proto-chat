...
export class LobeOllamaAI implements LobeRuntimeAI {
  ...
  async pullModel(params: PullModelParams, options?: ModelRequestOptions): Promise<Response> {
    ...
    const signal = options?.signal; // Get the incoming AbortSignal
    ...
    try {
      // Get the iterator for the Ollama pull operation
      ...
      // If an error occurs during client.pull call or stream creation, remove the listener
      signal?.removeEventListener('abort', abortOllama);
