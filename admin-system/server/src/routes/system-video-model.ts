...
/**
 * GET /api/admin/system-config/video-default-model - 获取当前视频默认模型配置
 */
router.get('/', authenticateToken, requirePermission('system.admin'), async (req, res) => {
  try {
    ...
    console.error('Get system video default model config error:', error);
    return res.status(500).json({
      message: '获取视频默认模型配置失败',
      success: false,
    });
  }
});
...
/**
 * POST /api/admin/system-config/video-default-model - 更新视频默认模型配置
 */
router.post('/', authenticateToken, requirePermission('system.admin'), async (req, res) => {
  try {
    ...
    if (!validation.success) {
      console.error('[VideoDefaultModel] Validation error:', validation.error.errors);
    ...
    return res.json({
      data: updatedConfig,
      message: '视频默认模型配置更新成功',
      success: true,
    });
  } catch (error) {
    console.error('Update system video default model config error:', error);
    return res.status(500).json({
      message: '更新视频默认模型配置失败',
      success: false,
    });
  }
});
